...
function Toast({ className, ...props }: ToastPrimitive.Root.Props) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(
        "group/toast pointer-events-auto absolute right-0 top-0 z-[calc(1000-var(--toast-index))] w-full origin-top rounded-none border bg-popover text-popover-foreground shadow-lg will-change-transform outline-none select-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50",
...
function info(title: string, description?: string) {
  toast.add({ type: "info", title, description, timeout: 4000 })
}

function warning(title: string, description?: string) {
  toast.add({ type: "warning", title, description, timeout: 5000 })
}
