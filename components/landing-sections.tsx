...
export function LandingSections({
  ...
  return (
    <>
      {/* SECTION 1: 4 ASPEK EVALUASI (EDITORIAL HUMAN STYLE) */}
      <section className="py-20 sm:py-24 bg-slate-50 dark:bg-[#0A0F1D] border-b border-slate-200 dark:border-slate-800/80">
            ...
            <div className="lg:col-span-7 space-y-4">
              {aspekList.map((aspek, index) => (
                <motion.div
                  ...
                  className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs flex gap-5 items-start transition-shadow hover:shadow-md"
                >
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-600 to-amber-600 text-white font-black text-base flex items-center justify-center shrink-0 shadow-sm mt-0.5">
                    {aspek.code}
                  </div>
                  <div className="space-y-1.5 flex-1">
                    ...
                    <p className="text-xs sm:text-sm text-slate-400 dark:text-slate-500 font-medium pt-1">
                      • {aspek.detail}
                    </p>
                  </div>
                </motion.div>
      ...
      {/* SECTION 2: ALUR TAHAPAN & MEKANISME KERJA */}
      <section className="py-20 sm:py-24 bg-white dark:bg-[#070B14]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-5">
            {alurSteps.map((step, index) => (
              <motion.div
                ...
                className="p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/60 shadow-xs space-y-3 flex flex-col justify-between"
              >
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-black text-amber-600 dark:text-amber-400 tracking-wider">
                      TAHAP {step.step}
                    </span>
                  </div>
                  <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-slate-100">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.5 }}
            className="p-8 sm:p-10 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/90 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6"
          >
            ...
            <Link
              ...
              className="shrink-0 inline-flex items-center gap-2 h-12 px-7 rounded-xl bg-red-700 hover:bg-red-800 text-white text-sm sm:text-base font-bold shadow-md shadow-red-950/20 transition-all hover:scale-[1.02]"
            >
