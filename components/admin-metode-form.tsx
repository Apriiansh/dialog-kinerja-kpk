...
import type { AdminMetodeFormState } from "@/lib/actions/admin-metode";
...
export function AdminMetodeForm({
  backHref,
  backLabel,
  submitLabel,
  action,
  ...
  backLabel: string;
  submitLabel: string;
  action: (formData: FormData) => Promise<AdminMetodeFormState>;
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    let result: AdminMetodeFormState | undefined;
    try {
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        <div className="flex flex-col gap-1.5">
          <label htmlFor="nama_metode" className={LABEL_CLASSES}>
            Nama Metode <span className="text-error">*</span>
          </label>
          <input
            id="nama_metode"
            name="nama_metode"
            type="text"
            maxLength={255}
            value={values.nama_metode ?? ""}
            onChange={(e) => setField("nama_metode", e.target.value)}
            placeholder="Contoh: Penugasan"
            className={INPUT_CLASSES}
            disabled={pending}
            aria-invalid={!!fieldErrors.nama_metode}
          />
          <p className="text-xs leading-5 text-ink-muted">
            Digunakan sebagai pilihan metode pengembangan pada item dialog
            kinerja.
          </p>
          {renderError("nama_metode")}
        </div>
        ...
        <label className="flex cursor-pointer items-start gap-2.5 text-sm text-ink">
          ...
          <span className="flex flex-col gap-0.5">
            ...
            <span className="text-xs font-normal leading-5 text-ink-muted">
              Metode aktif ditampilkan sebagai pilihan pada dialog kinerja.
            </span>
          </span>
        </label>
