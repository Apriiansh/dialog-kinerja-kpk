...
export type DialogReminderKind = "H_MINUS_1" | "H";
export type DialogReminderRecipient = "PEGAWAI" | "ATASAN";

function dialogReminderEmailHtml({
  recipient,
  recipientName,
  counterpartName,
  periode,
  ...
  recipient: DialogReminderRecipient;
  recipientName: string;
  counterpartName: string;
  periode: string;
  jadwalLabel: string;
  link: string;
  kind: DialogReminderKind;
}) {
  const isHMinus1 = kind === "H_MINUS_1";
  const isPegawai = recipient === "PEGAWAI";
  const headerTitle = isHMinus1
    ? "Dialog Kinerja Anda dilaksanakan besok"
    : "Dialog Kinerja Anda dilaksanakan hari ini";
  const bodyText = isPegawai
    ? isHMinus1
      ? `Jangan lupa, Dialog Kinerja periode <strong>${periode}</strong> Anda bersama atasan <strong>${counterpartName}</strong> akan dilaksanakan <strong>besok</strong>.`
      : `Dialog Kinerja periode <strong>${periode}</strong> Anda bersama atasan <strong>${counterpartName}</strong> dilaksanakan <strong>hari ini</strong>.`
    : isHMinus1
      ? `Jangan lupa, Dialog Kinerja periode <strong>${periode}</strong> antara Anda dan pegawai <strong>${counterpartName}</strong> akan dilaksanakan <strong>besok</strong>.`
      : `Dialog Kinerja periode <strong>${periode}</strong> antara Anda dan pegawai <strong>${counterpartName}</strong> dilaksanakan <strong>hari ini</strong>.`;
  const counterpartLabel = isPegawai ? "Atasan" : "Pegawai";

  return `
    <div style="margin:0;padding:0;background:#f5f2ec;font-family:Arial,Helvetica,sans-serif;">
      <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;">
        ${isHMinus1 ? "Besok" : "Hari ini"} pelaksanaan Dialog Kinerja periode ${periode}.
      </div>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f5f2ec;padding:32px 16px;">
        <tr>
          <td align="center">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:680px;background:#ffffff;border:1px solid #e7dfd3;border-radius:18px;overflow:hidden;box-shadow:0 14px 40px rgba(25,20,16,0.08);">
                ...
                <td style="background:#c8102e;padding:28px 32px;">
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                    <tr>
                      <td style="vertical-align:middle;">
                        <div style="display:inline-block;padding:7px 12px;border-radius:999px;background:rgba(255,255,255,0.16);color:#ffffff;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;">
                          Pengingat Dialog Kinerja
                        </div>
                        <div style="margin-top:14px;color:#ffffff;font-size:22px;line-height:1.25;font-weight:700;max-width:420px;">
                          ${headerTitle}
                        </div>
                        <div style="margin-top:8px;color:rgba(255,255,255,0.88);font-size:14px;line-height:1.6;max-width:480px;">
                          Jadwal pelaksanaan dialog kinerja yang telah disetujui.
                        </div>
                      </td>
                    </tr>
                  </table>
              ...
              <tr>
                <td style="padding:30px 32px 18px 32px;color:#241f1a;">
                  ...
                  <div style="font-size:14px;line-height:1.75;color:#4b443c;margin-bottom:22px;">
                    ${bodyText}
                    Silakan buka detail dialog kinerja untuk memastikan seluruh isian aspek sudah lengkap dan siap untuk dibahas.
                  </div>
                    ...
                    <tr>
                      <td style="padding:0 0 12px 0;">
                        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #ebe3d8;border-radius:14px;background:#fbf9f6;">
                          <tr>
                            <td style="padding:14px 16px 12px 16px;">
                              <div style="font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#8a7f74;margin-bottom:4px;">Jadwal Dialog</div>
                              <div style="font-size:15px;font-weight:700;color:#1f1914;line-height:1.5;">${jadwalLabel}</div>
                            </td>
                          </tr>
                        </table>
                      </td>
                    ...
                    <tr>
                      <td>
                        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #ebe3d8;border-radius:14px;background:#fbf9f6;">
                          <tr>
                            <td style="padding:14px 16px 12px 16px;">
                              <div style="font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#8a7f74;margin-bottom:4px;">${counterpartLabel}</div>
                              <div style="font-size:15px;font-weight:700;color:#1f1914;line-height:1.5;">${counterpartName}</div>
                            </td>
                          </tr>
                        </table>
                      </td>
                  ...
                  <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px 0;">
                    <tr>
                      <td align="center" style="border-radius:999px;background:#c8102e;">
                        <a href="${link}" style="display:inline-block;padding:13px 24px;font-size:14px;font-weight:700;line-height:1;color:#ffffff;text-decoration:none;">Buka Dialog Kinerja</a>
                      </td>
                    </tr>
                  </table>

                  <div style="font-size:12px;line-height:1.7;color:#7b7269;border-top:1px solid #ece3d7;padding-top:16px;">
                    Email ini dikirim otomatis oleh sistem Dialog Kinerja. Jika Anda merasa email ini bukan untuk Anda, abaikan pesan ini.
                  </div>
                </td>
              </tr>
            </table>
...
export async function sendDialogReminderEmail({
  to,
  recipient,
  recipientName,
  counterpartName,
  ...
  const isHMinus1 = kind === "H_MINUS_1";
  const subjectPrefix = isHMinus1
    ? "Pengingat: Dialog Kinerja Besok"
    : "Pengingat: Dialog Kinerja Hari Ini";
  return sendEmail({
    to,
    subject: `${subjectPrefix} (${periode}) - ${counterpartName} | Dialog Kinerja`,
    html: dialogReminderEmailHtml({
      recipient,
      recipientName,
      counterpartName,
      periode,
